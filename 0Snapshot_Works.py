...
# html parsing
	...
	cursor = conn.cursor()
	cursor.execute('CREATE TABLE IF NOT EXISTS SurfSnap(ID INTEGER PRIMARY KEY, WaveSize TEXT, Beach TEXT, WebSource TEXT, Summary TEXT, Wind TEXT)')
...
# iterates over parsed HTML
	for wave in wavez:
		...
		for beach in beaches:
			...
			conn = sqlite3.connect('SurfSend.db')
			cursor = conn.cursor()
			# cursor.execute("INSERT INTO SurfSnap VALUES (?, ?, ?, ?,?)", (wavesizex, beachnamex, web_src, summary, windxx))
			cursor.execute("INSERT INTO SurfSnap (Wavesize, Beach, WebSource, Summary, Wind) VALUES (?,?,?,?,?)", (wavesizex,beachnamex,web_src,summary,windxx,))
			conn.commit()
			cursor.close()
			conn.close()